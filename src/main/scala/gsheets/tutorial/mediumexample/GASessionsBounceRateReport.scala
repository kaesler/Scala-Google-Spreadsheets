...
final case class GASessionsBounceRateReport(info: Map[String, DataRow]) {
  def row(infoType: String): js.Array[CellValue] = {
    ...
    js.Array(
      infoType,
      theseInfo.sessionsInfo.desktop,
      theseInfo.sessionsInfo.tablet,
      theseInfo.sessionsInfo.mobile,
      theseInfo.sessionsInfo.total,
      theseInfo.bounceRateInto.desktop,
      theseInfo.bounceRateInto.tablet,
...
object GASessionsBounceRateReport {
  ...
  /** We include the global value here since it has to be a weighted average and the info
    * aren't there.
    */
  final case class BounceRateInfo(
    mobile: Double,
    desktop: Double,
    tablet: Double,
    global: Double

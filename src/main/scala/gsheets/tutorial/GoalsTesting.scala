...
object GoalsTesting {
  ...
  /** Finds all Google Analytics goals of all of your accounts and list them in the active
    * sheet.
    *
    * If you have many accounts, and many goals, this may fail because the request is too
    * big. If that's the case, you can change the arguments in the Goals.list method.
    *
  ...
  def findListOfGoals(): Unit = {
    ...
    if (goalsWithUrl.nonEmpty) {
      val rangeNotation =
        s"A1:${alphabet(goalsWithUrl.head.length - 1)}${goalsWithUrl.length}"

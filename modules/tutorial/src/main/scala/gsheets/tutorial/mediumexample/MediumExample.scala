...
object MediumExample:
  ...
  def gaSessionsBounceRateReport(
    ...
    val previousMonth =
      new js.Date(currentMonth.getFullYear.toInt, currentMonth.getMonth.toInt - 1)
    ...
    val previousMonthBounceRates =
      bounceRateInfo(previousMonthData, previousMonthSessions)
